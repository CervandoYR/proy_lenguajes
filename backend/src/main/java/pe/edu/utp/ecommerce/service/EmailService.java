...
public class EmailService {
    ...
    public void enviarCorreo(String destinatario, String asunto, String contenidoHtml) {
        ...
        try {
            ...
            log.error(
                    "CONSEJO DIAGNÓSTICO SMTP: Si es error 535 (auth), verifica tu Contraseña de Aplicación de Gmail. Si es Timeout, verifica que Railway permita salida por puerto 587 o prueba con puerto 465.");
        }
    }
    ...
    public void enviarCorreoConfirmacionPedido(Pedido pedido, Usuario usuario) {
        ...
        tablaDetalles.append(
                "<thead><tr><th align='left'>Descripción</th><th align='center'>Cant.</th><th align='right'>Precio</th><th align='right'>Subtotal</th></tr></thead>");
    ...
    private String construirLineaTiempo(Pedido.Estado estado) {
        ...
        boolean step2Done = estado == Pedido.Estado.EN_PROCESO || estado == Pedido.Estado.ENVIADO
    ...
    private String obtenerDireccionTexto(Pedido pedido, Usuario usuario) {
        if (pedido.getDireccion() != null) {
            ...
            return String.format("%s, %s, %s%s",
                    ...
                    (dir.getReferencia() != null && !dir.getReferencia().isBlank())
                            ? " (Ref: " + dir.getReferencia() + ")"
                            : "");
        }
     ...
     * - Estructura limpia de alta fidelidad, paleta neutra (#F5F5F7 fondo, #FFFFFF
     * tarjeta).
     * - Jerarquía tipográfica rigurosa, contraste optimizado sin saturación ni
     * íconos genéricos de IA.
    ...
    /**
     * Envío automático de alertas de inventario transaccional cuando un producto entra en stock crítico o se agota.
     */
    @Async
    public void enviarCorreoAlertaStock(pe.edu.utp.ecommerce.model.Producto producto) {
        String asunto = "⚠️ Alerta de Inventario — " + producto.getNombre();
        boolean agotado = producto.getStock() <= 0;
        String badgeColor = agotado ? "#DC2626" : "#D97706";
        String badgeBg = agotado ? "#FEF2F2" : "#FFFBEB";
        String badgeText = agotado ? "SIN STOCK / AGOTADO" : "ALERTA DE STOCK CRÍTICO";

        String cuerpo = String.format(
                "<p class='lead'>Atención al equipo de Inventario y Logística,</p>" +
                "<p>El sistema de monitoreo transaccional ha detectado un nivel de stock crítico o agotado para un componente en el catálogo activo:</p>" +
                "<div class='status-card' style='border-color: %s; background-color: %s; margin: 24px 0;'>" +
                "  <div class='status-title' style='color: %s;'>Estado actual del inventario</div>" +
                "  <div class='status-badge' style='color: %s;'>%s</div>" +
                "  <div class='status-desc' style='color: #515154;'><strong>Producto:</strong> %s (ID: #%d)</div>" +
                "</div>" +
                "<div class='info-box' style='margin-top: 24px;'>" +
                "  <div class='grid-2'>" +
                "    <div>" +
                "      <div class='label'>Stock Disponible Actual</div>" +
                "      <div class='value' style='color: %s; font-size: 22px; font-weight: 800;'>%d unidades</div>" +
                "    </div>" +
                "    <div>" +
        ...
        String adminEmail = (mailFrom != null && mailFrom.contains("<") && mailFrom.contains(">"))
                ? mailFrom.substring(mailFrom.indexOf('<') + 1, mailFrom.indexOf('>')).trim()
                : (mailFrom != null && !mailFrom.isBlank() ? mailFrom : "notificaciones@servitek.pe");

        enviarCorreo(adminEmail, asunto, envolverPlantilla("Alerta de Inventario Servitek", cuerpo));
    }
}
